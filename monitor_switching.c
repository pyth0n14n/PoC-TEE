...
#define MARK_PIN 19   // IO(3) on Arty board; for fault range, marking pin, target
...
static struct metal_pmp_config config_forbid = {
  .L = METAL_PMP_UNLOCKED,
  .A = METAL_PMP_NAPOT,
  .X = 0,
  .W = 0,
...
static struct metal_pmp_config config_allow = {
  .L = METAL_PMP_UNLOCKED,
  .A = METAL_PMP_TOR,
  .X = 1,
  .W = 1,
...
int init_pmp(void) {
  int rc;
  rc =  metal_pmp_set_region(pmp, 0, config_forbid, napot_addr((size_t) FLASH_MON, (size_t) ROM_SIZE));
  rc += metal_pmp_set_region(pmp, 1, config_forbid, napot_addr((size_t) RAM_MON, (size_t) RAM_LARGE_SIZE));
  rc += metal_pmp_set_region(pmp, 2, config_forbid, napot_addr((size_t) FLASH_SEP2, (size_t) ROM_SIZE));
  rc += metal_pmp_set_region(pmp, 3, config_forbid, napot_addr((size_t) RAM_SEP2, (size_t) RAM_SIZE));
  rc += metal_pmp_set_region(pmp, 4, config_forbid, napot_addr((size_t) FLASH_SEP3, (size_t) ROM_SIZE));
  rc += metal_pmp_set_region(pmp, 5, config_forbid, napot_addr((size_t) RAM_SEP3, (size_t) RAM_SIZE));
  rc += metal_pmp_set_region(pmp, 6, config_disable, 0);
  rc += metal_pmp_set_region(pmp, 7, config_allow, MEM_END);
  return rc;
}

int change_pmp(uint8_t caller_id, uint8_t callee_id){
  int rc;
  ...
  switch(caller_id) {
  case CTX_SEP1:
    rc =  metal_pmp_set_region(pmp, 6, config_rom, napot_addr((size_t) FLASH_METAL, (size_t) ROM_METAL_SIZE));
    rc += metal_pmp_set_region(pmp, 7, config_ram, napot_addr((size_t) RAM_METAL, (size_t) RAM_METAL_SIZE));
    break;
  case CTX_SEP2:
    rc =  metal_pmp_set_region(pmp, 2, config_forbid, napot_addr((size_t) FLASH_SEP2, (size_t) ROM_SIZE));
    rc += metal_pmp_set_region(pmp, 3, config_forbid, napot_addr((size_t) RAM_SEP2, (size_t) RAM_SIZE));
  ...
  case CTX_SEP2:
    ...
    break;
  case CTX_SEP3:
    rc += metal_pmp_set_region(pmp, 4, config_rom, napot_addr((size_t) FLASH_SEP3, (size_t) ROM_SIZE));
    rc += metal_pmp_set_region(pmp, 5, config_ram, napot_addr((size_t) RAM_SEP3, (size_t) RAM_SIZE));
    break;
  default:
    return 7;
  }
...
void syscall_from_u_handler(struct metal_cpu *cpu, int ecode)
{
  ...
  if(ecode == ECODE_SYSCALL_FROM_U_MODE) {
    ...
    // --- Process is callse -> backup to stack ---
      ...
      change_pmp(caller_id, callee_id);
      context[callee_id] = 1;
